...
jeu2048NEGATIVE::jeu2048NEGATIVE(int taille,int c,int m):jeux(taille,taille,m),nFusion(0),maxFusion(taille/2){
	...
	for (int i=0; i < taille*taille; i++){
		...
		//int value = (rand() % 2);
		(*grid[(pos1/row)*row+(pos2%columns)]).setValue((rand()%(columns))?2:-2?4:-4);
		(*grid[(pos2/row)*columns+(pos2%columns)]).setValue((rand()%(columns))?2:-2?4:-4);
 }
...
bool jeu2048NEGATIVE::aPerdu()const{
...
bool jeu2048NEGATIVE::aGagner()const{
...
void jeu2048NEGATIVE::newCase(){
	...
	int pos1=vector[generateRandomValue(vector.size())];
		  ...
		  (*grid[pos1/row*row+pos1%columns]) = rand()%2?-2:2?4:-4;
}
        }
    
	

char jeu2048NEGATIVE::randomMove(){
    char t[4]= {'u','d','l','r'};
    int r = std::rand() % 4;
    return t[r];
